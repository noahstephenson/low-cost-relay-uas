...
\begin{document}
\title{System Architecture and Mission Feasibility of a Multirotor Communications Relay}
...
\begin{abstract}
An airborne relay may restore a communications path that an obstruction blocks, but the carrier must hold the radio in position and supply its resources. This paper asks which parts of a small multirotor relay architecture limit a declared stationary service mission. The architecture assigns mission traffic to the payload, aircraft command to a separate carrier path, electrical power to the battery and regulated branch, and mechanical support to the airframe and mount. An assessment workflow traces those allocations through declared inputs, two coupled checks, and the evidence needed for a later physical test. A geometric visibility test and free-space link budget screen the outbound path. A constant-disk-loading hover model then closes battery, propulsion, and structural mass on energy-sized and power-sized branches. Its aggregate hover coefficient is fitted to five published NASA test points and checked against three commercial quadrotors excluded from that fit. With an assumed 25 W auxiliary load and manufacturer discharge conditions, the three comparisons reproduce published hover times within $-6.7$ to $+3.2$ percent. This checks part of the power and energy calculation; it does not validate the carrier structure or a relay aircraft. For a declared 0.20 kg, 14 W payload, the midpoint relay at 120 m has a reference link-margin boundary of 25.0 km of endpoint separation. Adding 12 dB of excess loss reduces that boundary to 6.3 km. The reference carrier reaches its exploratory rotor-size limit at 42.1 minutes and loses finite mass closure at 52.4 minutes. Its hover calibration supplies no separate operating-condition margin, and single-point refits move the first boundary between 33 and 49 minutes. At 10 km separation, a 30-minute dwell closes at 4.77 kg, a 45-minute dwell closes at 15.15 kg outside the size screen, and a 60-minute dwell has no finite solution. Of 90 baseline grid cases, 18 pass both checks, 6 exceed size limits, 6 have no closure, and 60 fail connectivity. Each failure points to a different architecture decision. These results support preliminary screening of outbound hover service; full-mission energy, packet performance, and physical verification remain unresolved.
\end{abstract}
\tableofcontents
%======================================================================
...
The architecture must account for four flows: mission traffic, aircraft command, electrical power, and mechanical support. The payload handles the first. The carrier provides the other three while holding the payload at the geometry needed for service. This paper asks where that allocation stops being plausible under the declared mission and aircraft assumptions. Section 2 defines the architecture and assessed interval. Section 3 traces the assumptions through link and carrier checks. Section 4 presents the worked case and sampled envelope, and Sections 5 and 6 explain the decisions and limits. An appendix gives the mass balance for reproduction.
...
This paper makes three contributions to a preliminary architecture assessment. First, it assigns the relay service and carrier responsibilities across four flows with a stated system boundary. Second, it connects fixed payload mass, power, and RF assumptions to separate link and aircraft checks for the same mission. Third, it traces connectivity failure, excessive finite size, and mathematical nonclosure to different decisions. An aggregate hover-power fit and published commercial hover times check part of the carrier calculation. The result is a reproducible engineering case under declared assumptions.
...
\caption{Relay architecture and the worked geometry. (a) The aircraft supports a black-box payload across two connections, mechanical retention and regulated power, and the operator commands the aircraft on a separate radio path. (b) At 10 km separation the ideal screen blocks the direct path and clears the relay path. The screen represents a declared scenario without measured terrain; the separate command path is an allocation without demonstrated fault isolation.}
\label{fig:arch}
\end{figure*}

Mission traffic passes from G through the relay payload to U. Aircraft command and health reporting belong to the carrier path, separate from that traffic. The battery supplies propulsion and avionics as well as the regulated payload branch. The mount transfers the payload load into the airframe. Each flow has a different failure consequence: a blocked mission path loses service, a command-path failure affects control, a power shortfall limits dwell, and an inadequate mount prevents safe support. Only the mission-link and power/mass consequences are calculated here.

Two design commitments follow from that split. The aircraft-to-payload boundary supplies regulated power and mechanical retention, which defines the intended accommodation boundary. Similar mass and power alone do not establish interchangeability; mounting, voltage, thermal behavior, and electromagnetic compatibility still require verification. The operator commands the carrier over its own radio path. This is a logical allocation rather than demonstrated fault isolation, since both information paths share one battery and airframe.
...
The assessment follows selected system definition and verification practices described in the NASA Systems Engineering Handbook \cite{nasaSEH}. It starts with the requested relay service, sets the system boundary, and allocates responsibilities before evaluating link and carrier resources. Figure~\ref{fig:workflow} shows how declared inputs enter both checks and how their results support a design decision. The independent numerical checker verifies the model's arithmetic and saved cases. No integrated aircraft was built or tested for this study, so product verification and operational validation remain future work. This is an analysis workflow, not a completed development lifecycle.

\begin{figure}[!tb]
\centering
\includegraphics[width=\columnwidth]{figs/architecture_assessment_workflow.pdf}
\caption{Architecture assessment workflow. The service need and four allocated flows lead to declared inputs, parallel link and carrier checks, and independent model review. Failure labels identify which architecture assumption or resource needs reconsideration. Installed-payload tests and operational validation remain outstanding.}
\label{fig:workflow}
\end{figure}
...
\begin{aligned}
m_b&=\max(m_E,m_P),\\
m_E&=\frac{(km+P_0)t}{ue},\\
m_P&=\frac{1.15(1.75^{3/2}km+P_0)}{p}.
\end{aligned}
\end{equation}
...
The repository carries 35 automated tests covering a hand-calculated hover case, branch logic, label order, calibration, and boundary calculations. A separate checker rebuilds every link from coordinates and solves the mass balance by bisection without using the production solver, then compares all saved records. The tests and checks pass at the commit cited in the Data Availability section. They confirm that the numbers follow from the stated equations. They do not establish physical aircraft behavior.
...
\vspace{2pt}
\begin{minipage}{\columnwidth}\footnotesize
Link margins stay at 7.97 and 10.02 dB in every row, so the differences come entirely from the aircraft. The growth factor shown is the energy-limited one; the power-limited branch stays at 0.570 throughout. \gate{pass} means the relay helps and the aircraft stays within the screening limits; this does not validate the design.
\end{minipage}
\end{table}
...
Neither check alone gives the same answer as both together. In the baseline scenario 30 of the 90 cases pass the link check and 54 pass the aircraft check, but only 18 pass both. The 54 aircraft passes include 36 cases where no usable link exists. This leaves the 30 link passes, of which 12 have an aircraft that is too large or has no solution. The separate counts answer different questions. Their intersection identifies the 18 cases that satisfy both; the counts are deterministic grid totals and give neither probabilities nor a comparison of prediction accuracy.
...
\caption{Relay height at which the first hop begins to clear, for the five screen cases. Thresholds above 120 m remove that altitude layer from service, while changes that stay between 60 and 120 m leave the sampled results unchanged. This comparison is geometric; no terrain model underlies it.}
\label{fig:thresh}
\end{figure}
...
The labels point to different parts of the architecture. A connectivity failure calls for reconsidering relay placement, radio assumptions, or the service requirement. More battery cannot repair that failure while the RF assumptions remain fixed. A finite result outside the size screen calls for reconsidering dwell or the carrier's battery, propulsion, and structural allowances. Mathematical nonclosure means those resource relations have no finite solution under the assumed scaling; relaxing a mass ceiling cannot create one. The separate carrier command path and payload mount also need interface and physical tests before the allocation can become a verified design.
...
The payload supplies common mass, DC, and RF assumptions to the two checks. Link margin does not resize the radio, and altitude does not change the fixed-density hover calculation. Consequently, the assessment can identify which allocation or resource binds without optimizing the radio, trajectory, and aircraft together. The trace from requested service to limiting assumption is the principal architecture result.
...
A useful relay path depends on the payload's mission-traffic function, while a supportable carrier depends on power and mechanical resources. This case study assigns those responsibilities, keeps aircraft command separate, and reports the limiting mechanism for each sampled mission. At the reference inputs, the midpoint relay has a 25.0 km link-margin boundary at 120 m height; 12 dB of added loss reduces it to 6.3 km. Carrier size reaches the exploratory rotor limit at 42.1 minutes, while mathematical closure ends at 52.4 minutes within the assumed model. These values include no separate operating-condition margin, and single-point hover refits span 33 to 49 minutes for the first boundary.

The 10 km example distinguishes a 4.77 kg, 30-minute pass from a 15.15 kg, 45-minute exclusion and a 60-minute nonclosure. Across 90 baseline cases, 18 pass both screens. These outcomes call for different changes to geometry, service duration, or carrier resources. The hover calibration and published-aircraft comparisons support only part of the resource calculation. Structural scaling, the command and payload interfaces, and full-mission performance remain unverified. The workflow identifies the next engineering question without claiming a flight-ready relay.
...
The computational baseline, declared inputs, calibration data, 1,890 case records, tests, and independent checker are archived at \url{https://github.com/noahstephenson/low-cost-relay-uas} under annotated tag \nolinkurl{aeroconf-2027-paper-2784-r3}. The baseline commit is recorded in the repository README. The manuscript source is in the submission directory under a Creative Commons Attribution 4.0 license.

\acknowledgements

The author used generative artificial-intelligence tools during the preparation of this paper, including assistance with drafting, editing, and the supporting analysis code and documentation. The author reviewed and verified all content and takes full responsibility for it.
...
\bibitem{nasaSEH} NASA, \emph{NASA Systems Engineering Handbook}, NASA/SP-2016-6105 Rev. 2, 2016. [Online]. Available: \url{https://www.nasa.gov/wp-content/uploads/2018/09/nasa_systems_engineering_handbook_0.pdf}
