...
\begin{abstract}
A communications relay can clear an obstructed path while imposing an aircraft demand that a small multirotor cannot sustain. This paper develops a reproducible architecture case study that separates two questions: whether an outbound relay path has sufficient received-level margin, and whether its carrier can support the payload for the required stationary dwell. A geometric visibility test and a free-space link budget define the communications screen. A constant-disk-loading hover model then closes the battery, propulsion, and structural mass balance on energy-sized and power-sized battery branches, using an aggregate hover coefficient fitted to five published NASA test points and checked against three commercial quadrotors held out of that fit. Under manufacturer conditions and an assumed 25 W auxiliary load, those three held-out aircraft reproduce published hover times within $-6.7$ to $+3.2$ percent, a limited consistency check that does not validate the proposed relay aircraft or its structural model. For the declared 0.20 kg, 14 W payload, the reference link-margin boundary is 25.0 km of endpoint separation at a midpoint relay height of 120 m; a deterministic 12 dB excess-loss case reduces it to 6.3 km. The reference carrier, which carries no operating-condition margin beyond the laboratory hover calibration, reaches its exploratory rotor-size limit at 42.1 minutes and has no finite mass closure at or above 52.4 minutes. The scatter of the individual calibration points alone moves the first boundary between 33 and 49 minutes. At a fixed 10 km separation, 30 minutes closes at 4.77 kg, 45 minutes at 15.15 kg outside the physical screen, and 60 minutes has no finite solution. Of 90 baseline grid cases, 18 pass both screens, 6 have excessive finite size, 6 have no closure, and 60 fail connectivity. Identifying which mechanism binds is what supports an early architecture decision. These results describe conditional outbound hover service; full-mission energy, packet performance, and physical verification remain unresolved.
\end{abstract}
\tableofcontents
%======================================================================
...
Mission traffic passes from G through the relay payload to U. The operator uses a separate path to command the carrier. The battery supplies the aircraft and the regulated payload branch. The mount carries the payload load. This split shows what the carrier must provide for the declared relay service.
...
The labels point to different parts of the architecture. A connectivity failure calls for reconsidering geometry or the link budget. More battery cannot repair that failure while the RF assumptions remain fixed. A finite result outside the size screen calls for reconsidering dwell, carrier assumptions, or the permitted physical envelope. Mathematical nonclosure means the assumed battery, power, and mass relationship has no solution; relaxing a mass ceiling cannot create one.
...
The 10 km example distinguishes a 4.77 kg, 30-minute pass from a 15.15 kg, 45-minute exclusion and a 60-minute nonclosure. Across 90 baseline cases, 18 pass both screens. The hover calibration and published-aircraft comparisons support a limited part of the calculation, while structural scaling and full-mission performance remain unverified. The proposed architecture assigns carrier command to a separate radio path and requires the aircraft to power and support the payload. The assessment gives a systems engineer an explainable way to identify the constraint that must change before advancing a relay concept.
...
The computational baseline, declared inputs, calibration data, 1,890 case records, tests, and independent checker are identified at \url{https://github.com/noahstephenson/low-cost-relay-uas} (annotated tag aeroconf-2027-paper-2784-r2; computational baseline at commit 4f9e2cde8c429e6b241d4a09dc5f8e7dbb877069). The manuscript source is in the repository's submission directory under a Creative Commons Attribution 4.0 license.
