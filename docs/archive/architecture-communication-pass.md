...
> **Historical record.** This document describes an earlier repository presentation state. Use the [current overview](../../README.md), [primary architecture](../architecture.md), and [engineering reference](../reference/README.md) for active information.
...
|---|---|---|---|
| **How the Relay UAS Extends Communications** | Outsider, owner | What is the system doing, and why are its own control and relayed traffic separate? | Current performers, control/relay information exchanges, command receiver, and black-box payload in `model/architecture.yaml` |
| **What Is Inside This Project?** | Outsider, systems engineer | What is inside the product boundary and what remains external? | Inner/outer boundaries in `model/system.yaml`; current performers and payload records |
...
| **What Happens If the Relay Degrades?** | Outsider, owner, systems engineer | How does relay degradation differ from loss of aircraft control? | Current modes and transitions, degraded scenario, recovery/status requirements, and open safety/evidence gaps |
...
| **What Has This Project Established?** | Outsider, owner, auditor | Which results are established, deliberately out of scope, or dependent on future decisions and evidence? | Model status, verification records, gaps, feasibility disposition, and decision state |
| **Exploratory Feasibility: Dwell vs Payload** | Outsider, owner, systems engineer | Did a credible physical region appear in the exploratory analysis? | Deterministic feasibility grid generated by `analysis/feasibility.py` |
...
`model/system.yaml` state title, audience, question, and source object references. Visible
...
after **Technical detail and traceability**, where they are useful for audit and
...
1. ordinary-language purpose, principal finding, and maturity;
2. canonical project-in-one-picture view;
3. short operational sequence and plain-language subsystem summary;
4. architecture and feasibility findings;
5. canonical engineering-status view;
6. future decisions, future evidence, and deliberate scope limits; and only then
7. configurations, requirements, traceability, model navigation, regeneration, and
   scope constraints.

Governance remains accessible in the technical portion but no longer displaces the
concept, result, or maturity in the opening.
...
limitations remain in `docs/reference/feasibility-analysis.md` and `analysis/results/`.
...
- `docs/reference/architecture-atlas.md`
- this report

Generated architecture figures:

- `docs/figures/project-in-one-picture.svg`
- `docs/figures/system-boundary.svg`
- `docs/figures/physical-architecture.svg`
- `docs/figures/power-resource-flow.svg`
- `docs/figures/command-data-flow.svg`
- `docs/figures/mission-sequence.svg`
- `docs/figures/degraded-behavior.svg`
- `docs/figures/configuration-evolution.svg`
- `docs/figures/engineering-status.svg`
...
`docs/reference/baseline.md` was regenerated as part of the required workflow and remained
